    ...
    <div class="container" style="max-width: 800px; margin: 0 auto;">
        ...
        <div class="card">
            ...
            <div class="header-section" style="margin-bottom: 25px; border-bottom: 1px solid #e2e8f0; padding-bottom: 15px;">
                <h2 style="margin: 0;">Tambah Produk 2</h2>
            </div>
            ...
            <form action="{{ route('produk2.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group" style="margin-bottom: 20px; display: flex; flex-direction: column; gap: 8px;">
                    ...
                    <input type="text" name="judul" value="{{ old('judul') }}" placeholder="Masukkan judul produk" style="padding: 10px 14px; border: 1px solid {{ $errors->has('judul') ? '#ef4444' : '#cbd5e1' }}; border-radius: 6px; font-size: 14px; outline: none;" onfocus="this.style.borderColor='#0ea5e9'" onblur="this.style.borderColor='{{ $errors->has('judul') ? '#ef4444' : '#cbd5e1' }}'" required>
                    @error('judul')
                ...
                <div class="form-group" style="margin-bottom: 20px; display: flex; flex-direction: column; gap: 8px;">
                    ...
                    <textarea name="isi" rows="5" placeholder="Masukkan deskripsi lengkap produk" style="padding: 10px 14px; border: 1px solid {{ $errors->has('isi') ? '#ef4444' : '#cbd5e1' }}; border-radius: 6px; font-size: 14px; outline: none; resize: vertical;" onfocus="this.style.borderColor='#0ea5e9'" onblur="this.style.borderColor='{{ $errors->has('isi') ? '#ef4444' : '#cbd5e1' }}'" required>{{ old('isi') }}</textarea>
                    @error('isi')
                        <small style="color: #ef4444; font-size: 12px; margin-top: -4px;"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</small>
                    @enderror
                </div>
                ...
                <div class="form-group" style="margin-bottom: 30px; display: flex; flex-direction: column; gap: 8px;">
                    <label style="font-weight: 600; color: #334155; font-size: 14px;">Gambar Produk <span style="color: #ef4444;">*</span></label>
                    <input type="file" name="gambar" style="padding: 10px 14px; border: 1px solid {{ $errors->has('gambar') ? '#ef4444' : '#cbd5e1' }}; border-radius: 6px; font-size: 14px; outline: none; background-color: #f8fafc;" onfocus="this.style.borderColor='#0ea5e9'" onblur="this.style.borderColor='{{ $errors->has('gambar') ? '#ef4444' : '#cbd5e1' }}'" required>
                ...
                <div class="form-actions" style="display: flex; gap: 12px; border-top: 1px solid #e2e8f0; padding-top: 20px;">
                    ...
                    <!-- Tombol Kembali -->
                    <a href="{{ route('produk2.index') }}" class="btn btn-edit" style="background-color: #64748b; border-color: #64748b; color: white; display: inline-flex; align-items: center; gap: 8px; text-decoration: none; font-weight: 600; padding: 10px 20px;">

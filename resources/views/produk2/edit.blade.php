    ...
    <div class="container" style="max-width: 800px; margin: 0 auto;">
        ...
        <div class="card">
            ...
            <div class="header-section" style="margin-bottom: 25px; border-bottom: 1px solid #e2e8f0; padding-bottom: 15px;">
                <h2 style="margin: 0;">Edit Produk 2</h2>
            </div>

            <!-- Form edit produk 2 -->
            <form action="{{ route('produk2.update', $produk2->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="form-group" style="margin-bottom: 20px; display: flex; flex-direction: column; gap: 8px;">
                    ...
                    <input type="text" name="judul" value="{{ old('judul', $produk2->judul) }}" placeholder="Masukkan judul produk" style="padding: 10px 14px; border: 1px solid {{ $errors->has('judul') ? '#ef4444' : '#cbd5e1' }}; border-radius: 6px; font-size: 14px; outline: none;" onfocus="this.style.borderColor='#0ea5e9'" onblur="this.style.borderColor='{{ $errors->has('judul') ? '#ef4444' : '#cbd5e1' }}'" required>
                    @error('judul')
                ...
                <div class="form-group" style="margin-bottom: 20px; display: flex; flex-direction: column; gap: 8px;">
                    ...
                    <textarea name="isi" rows="5" placeholder="Masukkan deskripsi lengkap produk" style="padding: 10px 14px; border: 1px solid {{ $errors->has('isi') ? '#ef4444' : '#cbd5e1' }}; border-radius: 6px; font-size: 14px; outline: none; resize: vertical;" onfocus="this.style.borderColor='#0ea5e9'" onblur="this.style.borderColor='{{ $errors->has('isi') ? '#ef4444' : '#cbd5e1' }}'" required>{{ old('isi', $produk2->isi) }}</textarea>
                    @error('isi')
                        <small style="color: #ef4444; font-size: 12px; margin-top: -4px;"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</small>
                    @enderror
                </div>
                ...
                <div class="form-group" style="margin-bottom: 20px; display: flex; flex-direction: column; gap: 8px;">
                    ...
                    <div style="max-width: 250px; border-radius: 8px; overflow: hidden; border: 1px solid #e2e8f0; background-color: #f8fafc; padding: 8px;">
                        @if($produk2->gambar)
                            <img src="{{ asset($produk2->gambar) }}" alt="Preview Gambar" style="width: 100%; height: auto; border-radius: 6px; display: block;">
                        @else

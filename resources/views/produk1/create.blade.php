        ...
        <div class="card">
            ...
            <form action="{{ route('produk1.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group" style="margin-bottom: 20px; display: flex; flex-direction: column; gap: 8px;">
                    ...
                    <input type="text" name="judul" value="{{ old('judul') }}" placeholder="Masukkan judul produk" style="padding: 10px 14px; border: 1px solid {{ $errors->has('judul') ? '#ef4444' : '#cbd5e1' }}; border-radius: 6px; font-size: 14px; outline: none;" onfocus="this.style.borderColor='#0ea5e9'" onblur="this.style.borderColor='{{ $errors->has('judul') ? '#ef4444' : '#cbd5e1' }}'">
                    @error('judul')
                ...
                <div class="form-group" style="margin-bottom: 20px; display: flex; flex-direction: column; gap: 8px;">
                    ...
                    <textarea name="isi" rows="5" placeholder="Masukkan deskripsi lengkap produk" style="padding: 10px 14px; border: 1px solid {{ $errors->has('isi') ? '#ef4444' : '#cbd5e1' }}; border-radius: 6px; font-size: 14px; outline: none; resize: vertical;" onfocus="this.style.borderColor='#0ea5e9'" onblur="this.style.borderColor='{{ $errors->has('isi') ? '#ef4444' : '#cbd5e1' }}'">{{ old('isi') }}</textarea>
                    @error('isi')
                        <small style="color: #ef4444; font-size: 12px; margin-top: -4px;"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</small>
                    @enderror
                </div>
